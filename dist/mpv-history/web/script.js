...
function mapHistory(history) {
  const slicedMoviesHistory = history.movies
    .reverse()
    .slice(0, config.web.maxEntries)
  const slicedSeriesHistory = history.series
    .reverse()
    .slice(0, config.web.maxEntries)
...
function mapSeriesHistory(history) {
  ...
  const sortedOutput = output.sort(function (a, b) {
    return b.entries[0].timestamp - a.entries[0].timestamp
  })
...
function renderHistory(mappedHistory) {
  ...
  mappedHistory.series.forEach(function (series, index) {
    ...
    if (index < config.web.numExpandedSeries) {
    ...
    spanNode.innerHTML =
      'Latest: <a href="mpv://' + highestPath + '">' + highestPrefix + '</a>'
...
async function getConfig() {
  const configResponse = await fetch('../config.json?ts=' + Date.now())
...
async function getHistory() {
  const historyResponse = await fetch(
    '../' + config.file.history + '?ts=' + Date.now(),
  )
